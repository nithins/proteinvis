...
#include <qtcolorpicker_widgets.h>
    ...
    : QPushButton(parent), popup(0), withColorDialog(enableColorDialog)
{
    ...
    // Set text
    //setText(tr("color picker"));
    firstInserted = false;
    
    useStandardColors = false;    
    standardColorsInserted = false;
...
{
  if( useStandardColors == true && standardColorsInserted == false)
  {
...
void QtColorPicker::setUseStandardColorsEnabled(bool enabled)
{
  useStandardColors = enabled;  
  
  setStandardColors();
 
}
bool QtColorPicker::useStandardColorsEnabled() const
{
  return useStandardColors;
}
